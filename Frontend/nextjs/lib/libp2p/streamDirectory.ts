...
export class BrowserStreamDirectory {
   ...
   */
  private handleAnnouncement(message: DirectoryMessage, fromPeer: string): void {
    if (!message.streamId || !message.metadata) return;

    console.log(`📢 Discovered stream: ${message.streamId} from ${fromPeer.slice(0, 20)}...`);
   ...
   * Handle heartbeat
   */
  private handleHeartbeat(message: DirectoryMessage, fromPeer: string): void {
   ...
   * Handle deregister
   */
  private handleDeregister(message: DirectoryMessage, fromPeer: string): void {
    if (!message.streamId) return;
   ...
   */
  private handleResponse(message: DirectoryMessage, fromPeer: string): void {
